...
_TRANSLATOR_SYS_PROMPT = (
    "Sos un traductor EN->ES (espanol latino neutro). "
    "Devolve solo el texto traducido, sin comillas ni comentarios. "
    "Respeta exactamente los placeholders (por ej. __PH_0__, %s, {0}, $NAME$, etiquetas <b>...</b>, secuencias \\n). "
    "No agregues ni quites espacios alrededor de placeholders. "
    "Si el texto tiene varios renglones, devolve la misma cantidad de renglones y en el mismo orden. "
    "No elimines guiones de dialogo al inicio de linea. "
    "Usa signos de apertura ¿ y ¡ cuando corresponda. "
    "Usa espanol latino neutro, claro y natural."
)

_BATCH_SYS_PROMPT = (
    "Sos un traductor EN->ES (espanol latino neutro). "
    "Vas a recibir un array JSON de strings; cada string es un bloque completo de subtitulo. "
    "Debes responder EXCLUSIVAMENTE con un array JSON de strings de la MISMA longitud. "
    "Cada item traducido debe preservar exactamente la cantidad de renglones (\\n) y su orden respecto del item original. "
    "Respeta exactamente los placeholders (por ej. __PH_0__, %s, {0}, $NAME$, etiquetas <b>...</b>, secuencias \\n). "
    "No agregues ni quites espacios alrededor de placeholders ni cambies el numero de elementos. "
    "No incluyas texto adicional, ni bloques de codigo, ni comentarios. "
    "Usa signos de apertura ¿ y ¡ cuando corresponda y espanol latino neutro, claro y natural."
)
    ...
    for attempt in range(5):
        try:
            ...
            if not isinstance(arr, list) or len(arr) != len(frozens):
                raise ValueError("longitud de array invalida")
...
# ====================== Reporte de costos (vive en translate_core) ======================

def print_cost_report(
    model: str,
    preferred_section: str = "Text tokens",
    preferred_tier: str = "Standard",  # Standard / Batch / Flex / Priority
    pricing_json_path: str = "openai_pricing.json",
    pricing_md_path: str = "openai_pricing.md",
) -> None:
    prompt, completion = get_token_totals()
    print("REPORTE:")
...
# Wrapper conveniente: hace traduccion y luego imprime el reporte.
def run_translation(
    in_path: str,
    out_suffix: str = "_es",
    dry_run: bool = False,
